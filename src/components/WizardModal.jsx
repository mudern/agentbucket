...
export default function WizardModal({ open, title, steps, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/30 px-4 py-8">
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 px-6 py-4">
          <div className="text-lg font-semibold text-slate-950 dark:text-slate-50">{title}</div>
          <button onClick={close} className="rounded-lg px-2 py-1 text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-slate-700 dark:text-slate-300">
            关闭
          </button>
        </div>
          ...
          <div className="flex items-center gap-3">
            {steps.map((item, index) => (
              <div key={item.label} className="flex flex-1 items-center gap-3">
                ...
                <div className="hidden text-sm font-medium text-slate-700 dark:text-slate-300 sm:block">{item.label}</div>
        ...
        <div className="flex items-center justify-between border-t border-slate-200 dark:border-slate-700 px-6 py-4">
          <button
            ...
            disabled={step === 0}
            className="rounded-xl bg-slate-100 dark:bg-slate-700 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-200 disabled:cursor-not-allowed disabled:opacity-40"
          >
            上一步
          </button>
          {step < steps.length - 1 ? (
